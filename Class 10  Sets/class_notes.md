...
- Each loop iteration processes one item.
...
Use a `for` loop when you want to access every item in a Set one by one.

---

# Topic 8 - Mini Project

## VFX Shot Status Analyzer

### Objective

Create a simple VFX Shot Status Analyzer using Sets.

The tool analyzes shot data and identifies:
...
`difference()` finds shots that are available in `all_shots` but are not available in `completed_shots`.
...
`intersection()` finds shots that exist in both Sets.
...
`union()` combines both Sets and removes duplicate values.
...
A `for` loop is used to display each Set item separately.
...
The same technique can be used to display pending, priority, and tracked shots.
...
> Note: The order of the shots may be different because Sets are unordered.

---

## Important Points

- `difference()` finds values that remain in the left Set after matching values are removed.
- `intersection()` finds common values between Sets.
- `union()` combines Sets and keeps unique values.
- A `for` loop accesses Set items one by one.
- Set output order may change.
- Duplicate values are automatically removed.
...
The VFX Shot Status Analyzer demonstrates how Sets can be used to compare and manage unique production data.

```text
difference()   -> Pending Shots
intersection() -> Common / Priority Completed Shots
union()        -> Combined Tracked Shots
for Loop       -> Display Set Items
```
